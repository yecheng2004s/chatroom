...
module.exports = function (session) {
  ...
  RedisStore.prototype.set = function (sid, sess, fn) {
    ...
    if (!store.disableTTL) {
      ...
      // multi.set(key, jsess).expire(key, ttl).exec(function (err, replies) {
      //   console.log("MULTI got " + replies + " replies,err:" + err);
      // });
      store.client.set(key, jsess, function (er) {
        if (er) return fn(er);
        store.client.expire(key, ttl, function (er2) {
          if (er2) return fn(er2);
